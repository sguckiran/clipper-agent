...
describe('clip length rules', () => {
  ...
  it('accepts windows within the default bound', () => {
    expect(isValidClipLength({ startSec: 0, endSec: CLIP_MIN_SEC })).toBe(true);
    expect(isValidClipLength({ startSec: 0, endSec: 30 })).toBe(true);
    expect(isValidClipLength({ startSec: 0, endSec: CLIP_MAX_SEC })).toBe(true);
  });

  it('rejects windows that are too short or too long', () => {
    expect(isValidClipLength({ startSec: 0, endSec: 14.9 })).toBe(false);
    expect(isValidClipLength({ startSec: 0, endSec: 60.1 })).toBe(false);
  });

  it('honours custom bounds', () => {
    expect(isValidClipLength({ startSec: 0, endSec: 12 }, 10, 20)).toBe(true);
    expect(isValidClipLength({ startSec: 0, endSec: 45 }, 10, 20)).toBe(false);
  });
});
